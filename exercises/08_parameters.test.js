...
test('08_parameters-4: catch non-specified params', () => {
  ...
  expect(resty(1, 2, 3, undefined, 5, undefined, 7, undefined, 9, 10).length,
  ).toBe(8)
})
...
test('08_parameters-6: is an actual array, unlike arguments', () => {
  ...
  expect(Object.getPrototypeOf(args) === Object.getPrototypeOf(rests),
  ).toBe(false)
...
test('08_parameters-7: it can default all arguments, optionally', () => {
  ...
  const myFunction = ({ name = 'Unknown', age = 0, favoriteBand ='None' } = {}) => {
