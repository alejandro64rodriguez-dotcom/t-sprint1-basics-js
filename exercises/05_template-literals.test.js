...
const prefixes = ['super-cool ', 'really ']
    return literalString.reduce((result, fixedText, i) => {
      const value = interpolatedParts[i]
      const prefix = prefixes[i] ?? ''
      return result + fixedText + (value !== undefined ? prefix + value : '')
    }, '') 
   }
  const noun = 'World'
  const emotion = 'happy'
